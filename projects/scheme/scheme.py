...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        rator = scheme_eval(first,env)
        check_procedure(rator)
        def temp(x):
            return scheme_eval(x, env)
        rands = rest.map(temp)
        return scheme_apply(rator, rands, env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if(len(formals) != len(vals)):
            raise SchemeError('len(formals) != len(vals)')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        name, para, body = target.first, target.second, expressions.second
        procedure = LambdaProcedure(para, body, env)
