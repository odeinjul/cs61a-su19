...
def replace_leaf(t, old, new):
    ...
    """
    "*** YOUR CODE HERE ***"
    if(label(t) == old and is_leaf(t) == True):
        return tree(new, [replace_leaf(br, old, new) for br in branches(t)])
    else:
...
def balanced(m):
    ...
    # print(left_length, left_weight, right_length, right_weight)
    if left_length * left_weight == right_weight * right_length:
        if is_mobile(m):
            for ends in [end(left_side), end(right_side)]:
                if is_weight(ends):
                    next
                elif not balanced(ends):
                    return False
            return True
    return False
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        return tree(total_weight(m),[totals_tree(t) for t in [left_end, right_end]])
###################
# Extra Questions #
###################
