...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        if not self.string in env.keys():
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        temp_operands = [opera.eval(env) for opera in self.operands]
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        par_copy = self.parent.copy()
        par_copy.update({p: a for p, a in zip(self.parameters, arguments)})
        return self.body.eval(par_copy)
